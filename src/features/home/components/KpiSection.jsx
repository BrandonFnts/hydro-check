...
const KpiSection = ({ node }) => {
  ...
  useEffect(() => {
    ...
    // Buffer que acumula palabras pendientes de mostrar
    let wordQueue = [];
    let displayedText = "";
    let typingInterval = null;

    const startTyping = () => {
      if (typingInterval) return; // Ya está corriendo
      setIsTyping(true);
      typingInterval = setInterval(() => {
        if (wordQueue.length > 0) {
          ...
          // Ya no hay palabras y el stream terminó
          clearInterval(typingInterval);
          typingInterval = null;
          setIsTyping(false);
        }
      }, 30); // 30ms por palabra = efecto fluido y rápido
    };

    // Ref para saber si el stream sigue activo
    ...
    const fetchAI = async () => {
      ...
      await streamAIInsight(
        ...
        (chunk) => {
          ...
          setLoadingAi(false);
          // Dividir chunk en palabras individuales, conservando espacios
